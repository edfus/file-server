...
async function setList(direntListId, path, auth) {
  const headers = {
  ...
  return fetch(
    ...
    .then(
      rawJSON => {
        ...
        const direntList = document.getElementById(direntListId);
        ...
        dataList.parentNode.replaceChild(newDataList, dataList);
        dataList.remove();
      }
    );
}
