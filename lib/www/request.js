...
class SetDirentList {
  ...
  static shallowSquash (list, newList) {
    const set = newList[Symbol.for("files")] || list[Symbol.for("files")];

    for (const foldername of Object.keys(list)) {
      if(!(foldername in newList)) {
        delete list[foldername];
      }
    }
...
class DirentListFetcher {
  ...
  updateDirentList(path, direntList) {
    ...
    if(list) {
      ...
      direntList.options.forEach(o => o.remove());
      direntList.appendChild(newOptions);
      return "ok";
    }
  }
  ...
  normalizePath(path = "/") {
    return path.trim().replace(/[\\\/]+/g, "/").replace(/^([^\/])/, "/$1");
  }
  ...
  _addFiles (path, filesArray) {
    ...
    if(!pathParts.length) // root
      return SetDirentList.shallowSquash(this.list, setDirentList);
    ...
    if(typeof list[lastPart] === "object") {
      return SetDirentList.shallowSquash(list[lastPart], setDirentList);
    } else {
  ...
  _setList(path, list) {
    ...
    return SetDirentList.shallowSquash(oldList, list);
  }
...
async function upload(file, destination, auth) {
  ...
  if(destination.endsWith("/")) {
    destination = destination.concat(file.name);
  }
