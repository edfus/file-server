class DirentList extends HTMLElement {
  constructor() {
    ...
    this._observer = new MutationObserver(mutationsList => {
      for (const mutation of mutationsList) {
        ...
        if(mutation.type === "childList" && mutation.addedNodes.length) {
          this._addOptions(mutation.addedNodes);
        }

        if(mutation.type === "childList" && mutation.removedNodes.length) {
          this._removeOptions(mutation.removedNodes);
        }
      }
    });
    ...
    if (this.children.length) {
      this._addOptions(this.children);
    }
  ...
  _reverse () {
    if(this._reverseObserver)
      return ;
    this._reverseObserver = new ResizeObserver(
      () => {
        if(this.getAttribute("reverse") !== null) {
    ...
    this._reverseObserver.observe(this);
  }
  ...
  remove() {
    ...
    this._reverseObserver && this._reverseObserver.disconnect();
